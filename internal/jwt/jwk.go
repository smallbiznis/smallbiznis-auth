...
// KeyManager ensures tenants always have an active signing key.
...
func (m *KeyManager) EnsureSigningKey(ctx context.Context, tenantID int64) (domain.OAuthKey, error) {
	key, err := m.repo.GetActiveKey(ctx, tenantID)
	if err == nil {
		return key, nil
	}
...
func (m *KeyManager) ActiveKey(ctx context.Context, tenantID int64) (domain.OAuthKey, error) {
	key, err := m.repo.GetActiveKey(ctx, tenantID)
	if err != nil {
...
// JWKS returns the public JSON Web Key Set for the tenant.
func (m *KeyManager) JWKS(ctx context.Context, tenantID int64) (jose.JSONWebKeySet, error) {
	key, err := m.EnsureSigningKey(ctx, tenantID)
	if err != nil {
