...
func (g *Generator) GenerateAccessToken(ctx context.Context, tenant domain.Tenant, user domain.User, scope, issuer string, providers []string) (string, error) {
	key, err := g.keys.EnsureSigningKey(ctx, tenant.ID)
	if err != nil {
	...
	stdClaims := gojwt.Claims{
		...
		Audience:  gojwt.Audience{tenant.Name},
		Issuer:    issuer,
...
func (g *Generator) ValidateAccessToken(ctx context.Context, tenantID int64, token, issuer string) (*gojwt.Claims, *AccessTokenClaims, error) {
	key, err := g.keys.ActiveKey(ctx, tenantID)
	if err != nil {
