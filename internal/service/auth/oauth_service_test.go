...
func TestOAuthService_IntrospectToken(t *testing.T) {
	...
	require.Equal(t, int64(1), introspect.TenantID)
}
...
func newOAuthTestHarness() *oauthTestHarness {
	...
	providerClient := &fakeProviderClient{}
	tenantRepo := &fakeTenantRepo{tenant: domain.Tenant{ID: 1, Name: "Tenant"}}
	...
	svc := NewOAuthService(providerRepo, stateStore, providerClient, tenantRepo, userRepo, tokenRepo, generator, cfg, zap.NewNop())
...
func (f *fakeProviderRepo) GetProvidersByTenant(ctx context.Context, tenantID int64) ([]domainoauth.OAuthProviderConfig, error) {
	if cfgs, ok := f.configs[tenantID]; ok {
		return cfgs, nil
	}
	return nil, domainoauth.ErrProviderNotFound
}

func (f *fakeProviderRepo) GetProviderByName(ctx context.Context, tenantID int64, name string) (*domainoauth.OAuthProviderConfig, error) {
	if cfgs, ok := f.configs[tenantID]; ok {
...
func (f *fakeTenantRepo) GetDomainByHost(context.Context, string) (domain.Domain, error) {
	return domain.Domain{}, nil
}
func (f *fakeTenantRepo) GetTenant(ctx context.Context, tenantID int64) (domain.Tenant, error) {
	return f.tenant, nil
}
func (f *fakeTenantRepo) GetTenantBySlug(context.Context, string) (domain.Tenant, error) {
	return f.tenant, nil
}
func (f *fakeTenantRepo) GetPrimaryDomain(context.Context, int64) (domain.Domain, error) {
	return domain.Domain{}, nil
}
func (f *fakeTenantRepo) GetBranding(context.Context, int64) (domain.Branding, error) {
	return domain.Branding{}, nil
}
func (f *fakeTenantRepo) ListAuthProviders(context.Context, int64) ([]domain.AuthProvider, error) {
	return nil, nil
}
func (f *fakeTenantRepo) GetPasswordConfig(context.Context, int64) (domain.PasswordConfig, error) {
	return domain.PasswordConfig{}, nil
}
func (f *fakeTenantRepo) GetOTPConfig(context.Context, int64) (domain.OTPConfig, error) {
	return domain.OTPConfig{}, nil
}
func (f *fakeTenantRepo) ListOAuthIDPConfigs(context.Context, int64) ([]domain.OAuthIDPConfig, error) {
	return nil, nil
}
...
func (f *fakeUserRepo) GetByEmail(ctx context.Context, tenantID int64, email string) (domain.User, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeUserRepo) GetByID(ctx context.Context, tenantID, userID int64) (domain.User, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeTokenRepo) GetByRefreshToken(ctx context.Context, tenantID int64, token string) (domain.OAuthToken, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	for _, t := range f.tokens {
		if t.TenantID == tenantID && t.RefreshToken == token {
			return t, nil
		}
	}
