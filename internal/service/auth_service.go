...
func (s *AuthService) PasswordGrant(ctx context.Context, tenantCtx *tenant.Context, email, password, scope, issuer string) (*TokenResponse, error) {
	ctx, span := s.startSpan(ctx, "AuthService.PasswordGrant")
	defer span.End()

	normalized := strings.ToLower(strings.TrimSpace(email))
	user, err := s.users.GetByEmail(ctx, tenantCtx.Tenant.ID, normalized)
	if err != nil {
	...
	resp, err := s.issueTokens(ctx, tenantCtx, user, scope, issuer, providers)
	if err == nil {
		s.audit("password.login.success", "tenant_id", tenantCtx.Tenant.ID, "user_id", user.ID)
	} else {
		span.RecordError(err)
	}
	return resp, err
...
// OTPGrant validates a time-based OTP using tenant configuration.
func (s *AuthService) OTPGrant(ctx context.Context, tenantCtx *tenant.Context, email, code, scope, issuer string) (*TokenResponse, error) {
	...
	resp, err := s.issueTokens(ctx, tenantCtx, user, scope, issuer, providers)
	if err == nil {
		s.audit("otp.login.success", "tenant_id", tenantCtx.Tenant.ID, "user_id", user.ID)
	} else {
		span.RecordError(err)
	}
	return resp, err
...
func (s *AuthService) RefreshGrant(ctx context.Context, tenantCtx *tenant.Context, refreshToken, scope, issuer string) (*TokenResponse, error) {
	...
	token, err := s.tokens.GetByRefreshToken(ctx, tenantCtx.Tenant.ID, refreshToken)
	...
	user, err := s.users.GetByID(ctx, tenantCtx.Tenant.ID, token.UserID)
	if err != nil {
		span.RecordError(err)
	...
	s.audit("refresh_token.success", "tenant_id", tenantCtx.Tenant.ID, "user_id", user.ID)
	return resp, nil
}

// AuthorizationCodeGrant redeems an authorization code.
func (s *AuthService) AuthorizationCodeGrant(ctx context.Context, tenantCtx *tenant.Context, code, redirectURI, scope, issuer string) (*TokenResponse, error) {
	if code == "" {
	...
	user, err := s.users.GetByID(ctx, tenantCtx.Tenant.ID, stored.UserID)
	if err != nil {
...
func (s *AuthService) issueTokens(ctx context.Context, tenantCtx *tenant.Context, user domain.User, scope, issuer string, providers []string) (*TokenResponse, error) {
...
func (s *AuthService) ValidateToken(ctx context.Context, tenantID int64, token, issuer string) (*gojwt.Claims, *jwt.AccessTokenClaims, error) {
	return s.jwt.ValidateAccessToken(ctx, tenantID, token, issuer)
}
